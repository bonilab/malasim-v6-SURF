...
public:
  ...
  static void create_dxg_cli_options(CLI::App &app, DxGAppInput &input) {
    ...
    app.add_option("-t,--therapies", input.therapies, "Therapy range (e.g., 0 1 2 ...)")
        ->expected(-1);
    app.add_option("--tl,--therapy_list", input.therapy_list,
                   "List of therapies (e.g., 0 1 2 ...)")
        ->expected(-1);
    app.add_option("-g,--genotypes", input.genotypes,
                   "List of genotypes (e.g., WT KEL1 KEL1/PLA1)")
        ->expected(-1);
    ...
    app.add_option("--nd", input.number_of_drugs_in_combination,
                   "Number of drugs in combination")
        ->default_val(1);
    app.add_flag("--art", input.is_art, "Use ART configuration");

    app.add_option("--dose", input.dosing_days, "EE dosing days (e.g., 0 1 2 ...)")
        ->expected(-1);
    ...
    app.add_flag("--recurrence-test", input.is_recurrence_test,
                 "Run recurrence test mode with per-person recrudescence CSV output");
  }
