...
protected:
  ...
  void set_tf_value_for_therapy(int therapy_id, double tf_value) {
    auto &current_tf = mdc->current_tf_by_therapy();
    if (current_tf.size() <= static_cast<std::size_t>(therapy_id)) {
      current_tf.resize(static_cast<std::size_t>(therapy_id) + 1, 0.0);
    }

    current_tf[therapy_id] = tf_value;
  }
...
TEST_F(MFTRebalancingStrategyTest, UpdateEndOfTimeStepAtUpdateTime) {
  ...
  EXPECT_EQ(strategy->latest_adjust_distribution_time,
            current_time + strategy->delay_until_actual_trigger);
}
