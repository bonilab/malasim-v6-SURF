...
protected:
    void SetUp() override {
        test_fixtures::setup_test_environment("test_input.yml", [](YAML::Node& config) {
            ...
            config["genotype_parameters"]["pf_genotype_info"][0]["genes"][0]["cnv_reversion_multiplier"] = 1.0;
            config["genotype_parameters"]["pf_genotype_info"][3]["genes"][0]["cnv_reversion_multiplier"] = 1.0;
        });
    ...
    Genotype* build_double_copy_genotype() {
        ...
        if (pf_genotype_str[4].empty() || pf_genotype_str[4][0].empty()
            || pf_genotype_str[13].empty() || pf_genotype_str[13][0].empty()) {
            return nullptr;
        }
...
TEST_F(GenotypeTest, PerformCnvReversionWithoutDrugsRevertsConfiguredGenes) {
    ...
    auto* reverted = genotype->perform_cnv_reversion(
...
TEST_F(GenotypeTest, PerformCnvReversionUsesGlobalFallbackWhenGeneMultiplierMissing) {
    test_fixtures::setup_test_environment("test_input.yml", [](YAML::Node& config) {
        config["genotype_parameters"]["mutation_probability_per_locus"] = 1.0;
        config["genotype_parameters"]["default_cnv_reversion_multiplier"] = 1.0;
        config["genotype_parameters"]["pf_genotype_info"][0]["genes"][0].remove("cnv_reversion_multiplier");
        config["genotype_parameters"]["pf_genotype_info"][3]["genes"][0].remove("cnv_reversion_multiplier");
    });
    Model::get_instance()->release();
    utils::MaSimAppInput cli_input;
    cli_input.input_path = "test_input.yml";
...
TEST_F(GenotypeTest, PerformMutationByDrugCanIncreaseIntermediateCopyNumber) {
    ...
    auto* mutated = genotype->perform_mutation_by_drug(
        Model::get_config(), &mock_random, Model::get_drug_db()->at(1).get(), 1.0);
